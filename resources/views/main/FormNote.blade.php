...
<body class="bg-gray-100">
    ...
    <nav class="fixed top-0 left-0 w-full bg-white shadow-md px-4 py-3 flex justify-between items-center z-50">
        <h1 class="text-lg font-bold" style="color: #FF9B00">Input Note Balita</h1>
        @auth
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-md p-8">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-center" style="color: #FF9B00">
                    Input Note Balita - {{ \Carbon\Carbon::parse($tanggalAktif)->format('d/m/Y') }}
                </h2>
            </div>
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr class="bg-white border-b">
                                <td colspan="4" class="py-4 px-6 text-center">Tidak ada data pemeriksaan Note Balita
                                    yang kosong</td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="inputModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4" id="modalTitle">Input Note Balita</h3>
                <form id="measurementForm" method="POST" class="space-y-4">
                    ...
                    <!-- Checkbox Biarkan Kosong -->
                    <div class="flex items-center">
                        <input id="kosongCheck" type="checkbox"
                            class="w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-400">
                        <label for="kosongCheck" class="ml-2 text-sm text-gray-700">
                            Biarkan kosong
                        </label>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const kosongCheck = document.getElementById('kosongCheck');

            form.addEventListener('submit', function(e) {
                // kalau dicentang → override input
                if (kosongCheck.checked) {
                    ketInput.value = "KOSONG";
                }
            });
        });
        ...
        function showInputForm(id, nama) {
            ...
            title.textContent = `Input Note Balita - ${nama}`;
            form.action = `/formNote/${id}/isi-note`;
