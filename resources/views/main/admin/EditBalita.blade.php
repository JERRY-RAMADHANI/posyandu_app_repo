...
<div class="min-h-screen p-6" style="background: linear-gradient(135deg, #FF9B00 0%, #FFE100 100%);">
    <div class="w-full mx-auto bg-white rounded-xl shadow-md p-8">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-center" style="color: #FF9B00">Data Balita</h2>
        </div>

        <!-- Search Bar -->
        <div class="mb-4">
            <form action="{{ route('edit.balita') }}" method="GET" class="flex gap-2">
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-white uppercase bg-orange-500">
                    <tr>
                        ...
                        <th scope="col" class="py-3 px-6">Nama Orang Tua</th>
                        <th scope="col" class="py-3 px-6">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($dataBalita as $data)
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="py-4 px-6">{{ $data->nama_ortu }}</td>

                        <td class="py-4 px-6">
                            <a href="{{ route('daftar.balita.edit', $data->id) }}" 
