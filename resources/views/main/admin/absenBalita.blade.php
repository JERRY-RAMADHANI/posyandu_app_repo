...
@section('content')
    <div class="min-h-screen p-6" style="background: linear-gradient(135deg, #FF9B00 0%, #FFE100 100%);">
        <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-md p-8">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-center" style="color: #FF9B00">Absensi Balita</h2>
            </div>

            <form action="{{ route('absen.balita.store') }}" method="POST" class="space-y-6"
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative">
                        ...
                        <!-- Search Results -->
                        <div id="reg_results"
                            class="absolute z-10 w-full bg-white border border-gray-300 shadow-lg rounded-lg mt-1 max-h-60 overflow-y-auto">
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function performSearch(search, resultContainer, type) {
                try {
                    const response = await fetch(`/SearchBalita?search=${search}&type=${type}`);
            ...
            function selectPerson(data) {
                ...
                // Format alamat jadi RT / RW
                let alamat = "";
                if (data.rt && data.rw) {
                    alamat = `RT ${data.rt} RW ${data.rw}`;
                }
